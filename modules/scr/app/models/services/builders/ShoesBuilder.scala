...
class ShoesBuilderImpl @Inject()(val shoesRepository: ShoesRepositoryImpl) extends ShoesBuilder {
  ...
  override def getFilters(look: Look, filterByWeather: String, filterByEvent: String): List[Shoes => LogicalBoolean] = {
    checkIn(look) :::
      getFiltersByWeather(filterByWeather) :::
      getFiltersByEvent(filterByEvent) :::
      getFiltersByLength(look.length, filterByEvent) :::
      getHeelsFiltersByEvent(filterByEvent)
  }
  ...
  private def getHeelsFiltersByEvent(event: String): List[Shoes => LogicalBoolean] = event match {
    case "celebrate" => List(heelsFilter())
    case "relax" => List(noHeelsFilter())
    case _ => List()
  }
}
